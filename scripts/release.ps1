...
$uApiVersionRange = Require-Property $properties "u_api_version_range"
$modName = Require-Property $properties "mod_name"
$releaseRemote = Optional-Property $properties "release_remote" "origin"
...
if ($uApiVersion -notmatch "^\d+\.\d+\.\d+(?:-[0-9A-Za-z][0-9A-Za-z.-]*)?$") {
    throw "u_api_version '$uApiVersion' is not supported. Expected an exact SemVer-like value, for example 3.0.0."
}
if ($uApiVersionRange -notmatch "^\[[^\s,]+,(?:[^\s,]+)?\)$") {
    throw "u_api_version_range '$uApiVersionRange' is not supported. Expected value like [3.0.0,4.0.0)."
}
...
Write-Host "Required U-API:    $uApiVersionRange"
