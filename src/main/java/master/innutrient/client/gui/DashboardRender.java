...
final class DashboardRender {
    ...
    static void drawTrimmed(GuiGraphics graphics, Font font, Component text, int x, int y, int maxWidth,
                            int color, boolean shadow) {
        if (maxWidth <= 0) return;
        List<FormattedCharSequence> parts = font.split(text, maxWidth);
        graphics.drawString(font, parts.isEmpty() ? FormattedCharSequence.EMPTY : parts.getFirst(),
            x, y, color, shadow);
    }
